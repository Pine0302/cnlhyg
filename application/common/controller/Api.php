...
class Api
{
    ...
    //type = 1 招聘小程序
    //type=2 社区小程序
    public function getGUserInfo($sess_key,$type=1){
       $user_cache = $this->redis->get($sess_key);
       if(!empty($user_cache)){
           return json_decode($user_cache,true);
       }else{
           $user_arr = [];
           $userQuery = Db::table('user');
           $user_arr = $userQuery
               ->where('sess_key','=',$sess_key)
               ->field('id,openid,nickname,username,avatar,gender,mobile,city_name,city_code,is_engineer,is_hr,is_agent,sess_key,lat,lng,session_key')
               ->find();
    ...
    //获取某个厂商company_id的驻场用户id
    //reeturn  [] /null
    public function getBindUsersByReCompanyId($re_company_id){
        $bind_list = Db::table('re_binduser')
            ->field('user_id')
    ...
    //获取某个厂商admin_id的驻场用户id
    //reeturn  [] /null
    public function getBindUsersByAdminId($admin_id){
        $bind_list = Db::table('re_binduser')
            ->field('user_id')
    ...
    //获取某个厂商admin_id & re_company_id的驻场用户id
    //reeturn  [] /null
    public function getBindUsersByAdminCompanyId($admin_id,$re_company_id){
        $bind_list = Db::table('re_binduser')
            ->field('user_id')
            ->where('admin_id','=',$admin_id)
            ->where('re_company_id','=',$re_company_id)
    ...
    //获取用户的上级用户
    public function getUpUserInfo($user_id){

        $up_user_info = Db::table('user_team')->where('low_user_id','=',$user_id)->find();

        return $up_user_info;
    }

    public function mapCode($wx_code){
        $code = config('webset.code_map');
        foreach($code as $kc=>$vc){
            if($kc==$wx_code){
                return $vc;exit;
            }
        }
