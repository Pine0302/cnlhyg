        ...
        if(!empty($sess_key)){
            try{
                ...
                $company_list = $companyQuery
                    ...
                    ->field('c.id,c.name,c.icon,l.name as lname,c.scale,c.financing,c.img')
                    ->page($page,$page_size)
                    ->select();
                ...
                foreach($company_list as $kc=>$vc){
                   ...
                   // unset($company_list[$kc]['label']);
                    if(empty($vc['img'])){
                        ...
                        $company_list[$kc]['icon'] = "https://".config('webset.server_name').$vc['img'];
                    }
                    //获取job_name
     ...
     /*   $ukey = isset($data['ukey']) ? $data['ukey'] : '';
        ...
        if(!empty($sess_key)){
            try{
                ...
                if(empty($id)){
                    ...
                    if(!empty($id)){
                        $company_info = $companyQuery
                            ->field('id,name,image,img,icon,coordinate,areas as area,address,instruction,status,financing,scale,re_line_id,city_name,city_code')
                            // ->where('user_id','=',$user_info['id'])
                            ->where('id','=',$id)
                            ->find();
                    }
                    ...
                    $company_info = $companyQuery
                        ->field('id,name,image,icon,img,coordinate,areas as area,address,instruction,status,financing,scale,re_line_id,city_name,city_code')
                        ->where('id','=',$id)
                        ->find();
                }
                ...
                $city_name ='';
                if(!empty($company_info['city_code'])){
                    $city_info = Db::table('areas')->where('areano','=',$company_info['city_code'])->find();
                    if(!empty($city_info)){
                        $city_name = $city_info['areaname'];
                    }
                }
                $company_info['label1'] = !empty($company_info['city_name']) ? $company_info['city_name'] : $city_name;
           ...
           //     unset($company_info['label']);
                if(empty($company_info['img'])){
                    $company_info['icon'] = "https://".config('webset.server_name').config('webset.default_company_icon');
                }else{
                    $company_info['icon'] = "https://".config('webset.server_name').$company_info['img'];
                }
