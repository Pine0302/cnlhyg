...
class Index extends Api
{
    ...
    public function getVersionInfo(){
        ...
        $dataVersionInfo = Db::table('re_version')->where('id','1')->find();
        if($dataVersionInfo['name']==$data['version']){
            $data = ['is_shenhe'=>1];
        }else{
            $data = ['is_shenhe'=>0];
        }
    ...
    public function changeShenheVersion(){
        $data = $this->request->request();
        $dataVersionInfo = Db::table('re_version')->where('id','1')->update(['name'=>$data['version']]);
