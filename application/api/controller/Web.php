...
//测试获取用户列表接口
    ...
    public function webset(){
        $withdraw_limit = config('webset.withdraw_limit');
        $open_status = config('webset.open_status');
        $data = [
            'config'=>[
    ...
    public function bannerList(){
        $data = Db::table('re_adv')
            ->order('sort desc')
            ->select();
        $imgList = array_map(function($banner){
            return "https://".config('webset.server_name').$banner['img'];
        },$data);
        $data = [
            'banner_list'=>$imgList
        ];
